...
Consequently, the first point `X` of our example that is right under the plant canopy receives in average `incident_light$Total[1]*100`% of the light coming from the atmosphere: `incident_light$rdroit[1]*100`% of the light directly from the sky (either direct or diffuse light) + `incident_light$rtransmis[1]*100`% of the light that is transmitted by the plant above.
...
surfAS = 1.0 - surfAO
raint = P_parsurrg * trg * (1 - (incident_light$Shaded * surfAO) - 
                              (incident_light$Sunlit* surfAS))
```

For our example, the crop would have intercepted `r round(raint,2)` MJ m-2 day-1 of radiation, with a PAR of `r P_parsurrg * trg` W m-2, an `rombre` of `r incident_light$Shaded`, an `rsoleil` of `r incident_light$Sunlit` and a relative surface of `r surfAO` and `r surfAS` for the shaded and sunlit component respectively.
