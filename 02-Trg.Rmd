...
Which means that trg for the dominated plant (`i > 1`) is computed as the global atmospheric radiation (`trg_bak` here), reduced by the average proportion of light transmitted by the sunlit area of the plane below the dominant plant (and above the dominated plant).
...
A comparison of the two was made using the [sticRs](https://github.com/VEZY/sticRs) package, from which a summary plot was extracted. The results are shown in Figure \@ref(fig:trgcomparison1).

![(\#fig:trgcomparison1)trg comparison](img/trg-computation.png)
