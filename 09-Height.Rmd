...
The second method could be much simpler to parameterize from field, however, both plants experience the same microclimate in STICS for intercrops, which give the exact same developmental stages, and therefore the same `height` if using `somcour`. This method can not be implement as long as both plants have the same `somcour`, because the plant dominance depends on the `height` difference between the two plants.

Hence, we will implement a computation of the plant `height` using the plant dry mass (`masec`), that will be corrected by water and nitrogen stresses.
